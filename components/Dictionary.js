...
export default function Dictionary() {
  ...
  const [dictionaryCategories, setDictionaryCategories] = useState([{ id: 'all', name: 'Semua Kategori' }]);
  ...
  // Kamus, Flashcard, Quiz, dan Favorit sekarang memakai vocabulary + bookmark yang sama.
  useEffect(() => {
    let cancelled = false;
    ...
    const loadDictionary = async () => {
      try {
        ...
        setDictionaryData(safeData);

        const categories = [...new Set(safeData.map((item) => item.category).filter(Boolean))]
          .sort((a, b) => a.localeCompare(b, 'id'))
          .map((name) => ({ id: name, name }));
        setDictionaryCategories([{ id: 'all', name: 'Semua Kategori' }, ...categories]);

        const idSet = new Set(safeData.map((item) => Number(item.id)));
        const wordToId = new Map(safeData.map((item) => [normalizeWordKey(item.id_lang), Number(item.id)]));
        const sharedStored = JSON.parse(localStorage.getItem('lingospace_bookmarks') || '[]');
        const legacyDictionaryStored = JSON.parse(localStorage.getItem('dictionary_bookmarks') || '[]');

        const normalizeSharedId = (value) => {
          const numeric = Number(value);
          if (Number.isInteger(numeric) && idSet.has(numeric)) return numeric;
          return wordToId.get(normalizeWordKey(value));
        };

        const sharedIds = Array.isArray(sharedStored)
          ? sharedStored.map(normalizeSharedId).filter(Number.isInteger)
          : [];
        const legacyIds = Array.isArray(legacyDictionaryStored)
          ? legacyDictionaryStored
              .map((value) => Number(legacyDictionaryIdMap[String(value)]))
              .filter((value) => Number.isInteger(value) && idSet.has(value))
          : [];
        ...
        if (!cancelled) {
          setDictionaryData([]);
          setDictionaryCategories([{ id: 'all', name: 'Semua Kategori' }]);
          setBookmarks([]);
        }
      } finally {
  ...
  const toggleBookmark = (id) => {
    ...
    const newBookmarks = bookmarks.includes(numericId)
      ? bookmarks.filter((bookmarkId) => bookmarkId !== numericId)
      : [...bookmarks, numericId];
  ...
  // Copy to clipboard dengan fallback untuk browser lama
  const copyToClipboard = async (text, id) => {
    try {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        ...
        // Fallback untuk browser lama (Android 8 ke bawah)
  ...
  const playAudio = (text, lang) => {
    ...
    if ('speechSynthesis' in window) {
      ...
      // Fallback untuk browser yang tidak support speechSynthesis
  ...
  return (
    <div className="animate-fade-in max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
      ...
      <div className="text-center mb-6 sm:mb-8 animate-slide-down">
        ...
        <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold mb-2 bg-gradient-to-r from-purple-400 via-pink-400 to-blue-400 bg-clip-text text-transparent animate-gradient">
          Kamus Tematik 3 Bahasa
        </h2>
        <p className="text-gray-400 text-sm sm:text-lg">Indonesia • English • العربية</p>
        <div className="mt-4 flex justify-center gap-2 flex-wrap">
          <span className="px-3 py-1 rounded-full bg-purple-500/20 text-purple-300 text-xs sm:text-sm">
            {dictionaryData.length} Kata
          </span>
          <span className="px-3 py-1 rounded-full bg-pink-500/20 text-pink-300 text-xs sm:text-sm">
      ...
      <div className="glass-modern rounded-2xl p-3 sm:p-6 mb-6 sm:mb-8 animate-fade-in shadow-2xl">
        <div className="flex flex-col md:flex-row gap-3 sm:gap-4">
          ...
          <div className="relative">
            <select
              ...
              className="w-full md:w-auto px-4 sm:px-6 py-2.5 sm:py-3 rounded-full glass-modern bg-white/5 outline-none text-white cursor-pointer focus:ring-2 focus:ring-purple-500 transition-all appearance-none pr-10 sm:pr-12 text-sm sm:text-base"
            >
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 md:flex-none px-3 sm:px-4 py-2.5 sm:py-3 rounded-full transition-all transform hover:scale-105 text-sm sm:text-base ${
                viewMode === 'grid' 
        ...
        <div className="mt-3 sm:mt-4 text-xs sm:text-sm text-gray-400 animate-fade-in">
          Menampilkan <span className="text-purple-400 font-bold text-sm sm:text-lg">{filteredData.length}</span> dari {dictionaryData.length} kata
        </div>
      </div>

      {/* Dictionary Content dengan Animasi */}
      {viewMode === 'grid' ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {filteredData.map((item, index) => (
            ...
            >
              ...
              {/* Contoh Kalimat (Collapsible di Mobile) */}
              {item.example_id && (
                <div className="border-t border-white/10 pt-3 sm:pt-4 mb-3 sm:mb-4 bg-white/5 rounded-lg p-2 sm:p-3">
                  ...
                  >
                    ...
                    <span className="text-xs sm:text-sm">{expandedCard === item.id ? '−' : '+'}</span>
                  </button>
                  
                  {(expandedCard === item.id || window.innerWidth >= 768) && (
        ...
        <div className="space-y-3 sm:space-y-4">
          {filteredData.map((item, index) => (
            ...
            >
              ...
              {item.example_id && (
                <div className="border-t border-white/10 pt-3 mb-3 bg-white/5 rounded-lg p-2 sm:p-3">
                  <p className="text-[10px] sm:text-xs text-gray-400 mb-2">💬 Contoh:</p>
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <button onClick={() => playAudio(item.example_id, 'id')} className="flex-shrink-0 px-2 py-1 rounded-full glass-modern text-[10px] sm:text-xs hover:bg-purple-500/40 transition-all">🔊 ID</button>
                      <p className="text-xs sm:text-sm text-gray-300 flex-1">{item.example_id}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <button onClick={() => playAudio(item.example_en, 'en')} className="flex-shrink-0 px-2 py-1 rounded-full glass-modern text-[10px] sm:text-xs hover:bg-purple-500/40 transition-all">🔊 EN</button>
                      <p className="text-xs sm:text-sm text-blue-300 flex-1">{item.example_en}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <button onClick={() => playAudio(item.example_ar, 'ar')} className="flex-shrink-0 px-2 py-1 rounded-full glass-modern text-[10px] sm:text-xs hover:bg-purple-500/40 transition-all">🔊 AR</button>
                      <p className="text-xs sm:text-sm text-purple-300 flex-1 text-right" dir="rtl">{item.example_ar}</p>
                    </div>
                  </div>
                </div>
